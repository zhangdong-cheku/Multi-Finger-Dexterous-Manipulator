...
const int OPEN_ANGLE  = 85;      // 全部张开
const int CLOSE_ANGLE_L = 170;     //左
const int CLOSE_ANGLE_R = 0;     //右
  ...
  if (Serial.available() > 0) {
    char c = Serial.read();
      ...
      if (c == '1') {
        ...
        moveAllToAngle(CLOSE_ANGLE_R);
        allClosed = true;
      }
      if (c == '2') {
        Serial.println("命令：全部抓紧");
        moveAllToAngle(CLOSE_ANGLE_L);
        allClosed = true;
      }
      if (c == '3') {
        Serial.println("命令：全部抓紧");
...
void moveAllToAngle_01() {
  ...
  //   delay(20);   // 每个舵机之间稍微错峰一下，减小瞬间电流
  // }
    setServoAngle(SERVO_CHANNELS[0], 170);
    setServoAngle(SERVO_CHANNELS[1], 0);
    setServoAngle(SERVO_CHANNELS[2], 170);
    setServoAngle(SERVO_CHANNELS[3], 0);
    
    setServoAngle(SERVO_CHANNELS[4], 170);
    setServoAngle(SERVO_CHANNELS[5], 0);
    setServoAngle(SERVO_CHANNELS[6], 170);
    setServoAngle(SERVO_CHANNELS[7], 0);
    setServoAngle(SERVO_CHANNELS[8], 170);
    setServoAngle(SERVO_CHANNELS[9], 0);
}
